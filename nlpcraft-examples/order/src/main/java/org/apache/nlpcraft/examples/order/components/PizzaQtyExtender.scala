...
class PizzaQtyExtender extends NCEntityMapper:
    ...
    override def map(req: NCRequest, cfg: NCModelConfig, entities: util.List[NCEntity]): util.List[NCEntity] =
        ...
        if pizzas.nonEmpty && nums.nonEmpty then
            if pizzas.size != nums.size then throw new NCRejection("Pizza and their nums should be defined together1")
            var ok = true
            val mapped =
                for ((e1, e2) <- pizzas.zip(nums) if ok) yield
                    if e1.getId == e2.getId then
                        ...
                        val (pizza, num) = if e1.getId == "ord:pizza" then (e1, e2) else (e2, e1)
                        new NCPropertyMapAdapter with NCEntity:
                            // Copy from pizza.
                            pizza.keysSet().forEach(k => put(k, pizza.get(k)))
                            // New value from size.
                            put[String]("ord:pizza:qty", num.get[String]("stanford:number:nne").toLowerCase)

                            override val getTokens: JList[NCToken] = (extract(pizza) ++ extract(num)).sortBy(_.getIndex).asJava
