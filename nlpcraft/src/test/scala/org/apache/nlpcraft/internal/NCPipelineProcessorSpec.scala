...
class NCPipelineProcessorSpec:
    ...
    def test(): Unit =
        def test(txt: String, variantCnt: Int, elements: NCSemanticElement*): Unit =
            ...
            val parser = new NCSemanticEntityParser(new NCEnPorterStemmer, pipeline.getTokenParser, elements.asJava)
    ...
    /**
      ...
      * @return
      */
    private def mkSlowPipelineProcessor(delayMs: Long, itersCnt: Int): NCPipelineProcessor =
        val pipeline = EN_PIPELINE.clone()
        ...
        (0 until itersCnt).foreach(i => pipeline.getEntityParsers.add(mkSlowParser(i)))
    ...
    def testTimeout(): Unit =
        ...
        try
            Assertions.assertThrows(classOf[TimeoutException], () => fut.get(1, TimeUnit.MILLISECONDS))
        finally
            fut.cancel(true)
