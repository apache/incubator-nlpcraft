...
object NCProbeEnrichmentManager extends NCService with NCOpenCensusModelStats {
    ...
    private def ask0(
        ...
        tbl += (s"${b("  First Name$ansiReset")}", senMeta.getOrElse("FIRST_NAME", ""))
        tbl += (s"${b("  Last Name$ansiReset")}", senMeta.getOrElse("LAST_NAME", ""))
        tbl += (s"${b("  Email$ansiReset")}", senMeta.getOrElse("EMAIL", ""))
        tbl += (s"${b("  Company Name$ansiReset")}", senMeta.getOrElse("COMPANY_NAME", ""))
        tbl += (s"${b("  Is Admin$ansiReset")}", senMeta.getOrElse("IS_ADMIN", ""))
        tbl += (s"${b("  Signup Date$ansiReset")}", new Date(java.lang.Long.parseLong(senMeta("SIGNUP_TSTAMP").toString)))
        tbl += (s"${b("User Agent$ansiReset")}", senMeta.getOrElse("USER_AGENT", ""))
        tbl += (s"${b("Remote Address$ansiReset")}", senMeta.getOrElse("REMOTE_ADDR", ""))
        tbl += (s"${b("Server Timestamp$ansiReset")}", new Date(java.lang.Long.parseLong(senMeta("RECEIVE_TSTAMP").toString)))
