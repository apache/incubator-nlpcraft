...
private[impl] object NCEnStopWordsFinder:
    ...
    private case class ScanHolder(
        ...
        private def filterPoses(m: Map[String, Set[Wildcard]]): Set[String] =
            m.filter { case(_, pair) => pair.exists { (begin, end) => begin.isEmpty && end.isEmpty } }.keySet
    ...
    private def readStopWords(lines: Seq[String]): Map[Boolean, StopWordHolder] =
        ...
        for (line <- lines)
            def throwError(msg: String): Unit =
                throw new NCException(s"Invalid stop word configuration [line=$line, reason=$msg]")
